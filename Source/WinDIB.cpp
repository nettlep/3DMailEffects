...
unsigned char	winDIB::blendTable8[256][256];
bool		winDIB::blendTableInitialized;
		...
		:_dc(dc), _frameBuffer(NULL), _zBuffer(NULL)
{
	...
	if (!blendTableInitialized)
	{
		for (unsigned int i = 0; i < 256; i++)
		{
			...
			{
				blendTable8[i][j] = (unsigned char) ((i*j) >> 8);
			}
		}
...
unsigned int	winDIB::scaleAlpha(const unsigned int color, const int scale)
{
...
unsigned int	winDIB::alphaBlend(const unsigned int dst, const unsigned int src)
{
...
void		winDIB::line(const float x0, const float y0, const float x1, const float y1, const unsigned int color)
{
	...
	unsigned long	lx0 = (long) (cx0 * (float) F_ONE);
	unsigned long	ly0 = (long) (cy0 * (float) F_ONE);
	unsigned long	lx1 = (long) (cx1 * (float) F_ONE);
	unsigned long	ly1 = (long) (cy1 * (float) F_ONE);
	...
	unsigned long	aDX = (unsigned long) dx;
	unsigned long	aDY = ly1 - ly0;

	// The line starts here

	unsigned int	pitch = width();
	unsigned int	*fb = (unsigned int *) frameBuffer() + (ly0 >> F_BITS) * pitch + (lx0 >> F_BITS);
	...
	if (aDX > aDY)
	{
		...
		unsigned long	dy = (unsigned long) ((float) aDY / (float) aDX * (float) F_ONE);
	...
	else
	{
		...
		unsigned long	dx = (unsigned long) ((float) aDX / (float) aDY * (float) F_ONE);
...
void	winDIB::fillRect(const int left, const int top, const int right, const int bottom, const unsigned int color)
{
	...
	if (l >= (int) width() || r < 0 || t >= (int) height() || b < 0) return;
	...
	if (r >= (int) width()) r = width() - 1;
	if (b >= (int) height()) b = height() - 1;
	...
	if (alpha == 0xff)
	{
		unsigned int	*fbRow = (unsigned int *) frameBuffer() + y0 * width() + x0;
