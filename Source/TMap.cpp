...
#include <stdafx.h> // If this produces an error, feel free to remove it
#include "3Dme.h"
#include "TMap.h"
#include "Bitmap.h"
...
static	const	unsigned int	textureWidth = 256;		// Texture size
static	const	unsigned int	textureHeight = 256;		//

static	const	unsigned int	subShift = 4;			// Sub-affine span size
static	const	unsigned int	subSpan = 1 << subShift;	//
...
void	drawAffineTexturedPolygon(sVERT *verts, unsigned int *frameBuffer, unsigned int *textureBuffer, float *zBuffer, const unsigned int pitch)
{
	...
	while(!done)
	{
		...
		while(height-- > 0)
		{
			...
			for (; start < end; start++)
			{
				if (w > *zspan)
				{
					*span = textureBuffer[((iv>>8) & 0xFF00) + ((iu>>16) & 0xFF)] | 0xff000000;
					*zspan = w;
				}
				span++;
				zspan++;
...
// ---------------------------------------------------------------------------------------------------------------------------------
// Same as the routine above, but with AA built in...
// ---------------------------------------------------------------------------------------------------------------------------------

void	drawAffineTexturedPolygonAA(sVERT *verts, unsigned int *frameBuffer, unsigned int *textureBuffer, float *zBuffer, const unsigned int pitch)
{
	// Find the top-most vertex
	...
	while(!done)
	{
		...
		while(height-- > 0)
		{
			...
			float		overWidth = 1.0f / (re.x - le.x + 1);
			float		du  = (re.u - le.u) * overWidth;
			float		dv  = (re.v - le.v) * overWidth;
			float		dw  = (re.w - le.w) * overWidth;
			int		idu = (int) (du * 65536.0f);
			...
			unsigned int	*span = fb + start;
			float		*zspan = zb + start;
			unsigned int	*sSpan = span;
			unsigned int	*eSpan = span + (end - start);

			for (; start < end; start++)
			{
				if (w > *zspan)
				{
					*span = textureBuffer[((iv>>8) & 0xFF00) + ((iu>>16) & 0xFF)] | (span[0]&0xff000000);
					*zspan = w;
				}
				span++;
				zspan++;
			...
			// Anti-aliasing requires the last pixel to be drawn...
			{
				float		subTex = (float) _end - re.x;
				int		iu = (int)((re.u - du * subTex) * 65536.0f);
				int		iv = (int)((re.v - dv * subTex) * 65536.0f);
				float		w  =       (re.w - dw * subTex);
			...
			if (_start < _end)
			{
				unsigned int	lspan = sSpan[0] >> 24;
				float		lAdd = ((float) _start - le.x) * 255.0f;
				lspan += (unsigned int) lAdd;
				if (lspan > 255) lspan = 255;
				sSpan[0] = (sSpan[0]&0xffffff) | (lspan << 24);

				unsigned int	rspan = eSpan[0] >> 24;
				float		rAdd = ((float) _end - re.x) * 255.0f;
				rspan += 255 - (unsigned int) rAdd;
				if (rspan > 255) rspan = 255;
				eSpan[0] = (eSpan[0]&0xffffff) | (rspan << 24);

				sSpan++;
				for (; sSpan < eSpan; sSpan++)
				{
					*sSpan |= 0xff000000;
				}
			}
...
void	drawPerspectiveTexturedPolygon(sVERT *verts, unsigned int *frameBuffer, unsigned int *textureBuffer, float *zBuffer, const unsigned int pitch)
{
	// Find the top-most vertex
...
void	drawSubPerspectiveTexturedPolygon(sVERT *verts, unsigned int *frameBuffer, unsigned int *textureBuffer, float *zBuffer, const unsigned int pitch)
{
	// Find the top-most vertex
	...
	while(!done)
	{
		...
		while(height-- > 0)
		{
			...
			for(; start < end; start += subSpan)
			{
				...
				int		len = _min(subSpan, const_cast<const unsigned int &>(l));
				pixelsDrawn += len;
				...
				for (int j = 0; j < len; j++)
				{
					if (w0 > *zspan)
					{
						*span = textureBuffer[((t>>18)&0xFC0)+((s>>24) & 0x3f)];
						*zspan = w0;
					}
					s += ds;
					t += dt;
