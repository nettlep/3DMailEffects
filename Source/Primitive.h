...
inline	const	unsigned int	vertexCount() const	{return vertices().size();}
inline	const	vector<vert<T> >&vertices() const	{return _vertices;}
inline		vector<vert<T> >&vertices()		{return _vertices;}
