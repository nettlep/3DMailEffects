...
void	Logger::start(const bool reset)
{
	...
	limitFileSize();
	ofstream of(logFile().c_str(), ios::out|ios::binary | (reset ? ios::trunc : ios::app));
	if (!of) return;
...
void	Logger::stop()
{
	...
	limitFileSize();
	ofstream of(logFile().c_str(), ios::out|ios::binary|ios::app);
	if (!of) return;
...
void	Logger::logTex(const string &s, const LogFlags logBits)
{
	...
	of << headerString(logBits) << s << "\r\n";
}

// ---------------------------------------------------------------------------------------------------------------------------------

void	Logger::logRaw(const string &s, const LogFlags logBits)
{
	if (!(logBits & logMask())) return;
...
void	Logger::indent(const string &s, const LogFlags logBits)
{
	...
	if (lineCharsFlag())	of << headerString(logBits) << "\xDA\xC4\xC4" << s << "\r\n";
	else			of << headerString(logBits) << "+-  " << s << "\r\n";
...
void	Logger::undent(const string &s, const LogFlags logBits)
{
	...
	if (lineCharsFlag())	of << headerString(logBits) << "\xC0\xC4\xC4" << s << "\r\n";
	else			of << headerString(logBits) << "+-  " << s << "\r\n";
}
...
const	string	&Logger::headerString(const LogFlags logBits) const
{
	...
	string	version = wtoa(VERSION_STRING) + "-" + integerToString(getCurrentPatchLevel());
	sprintf(temp, "%s %15s[%04d]", version.c_str(), sourceFile().substr(ix).c_str(), sourceLine());
	headerString += temp;
	...
	sprintf(temp, "%02d/%02d %02d:%02d:%02d ", tme->tm_mon + 1, tme->tm_mday, tme->tm_hour, tme->tm_min, tme->tm_sec);
	headerString += temp;
