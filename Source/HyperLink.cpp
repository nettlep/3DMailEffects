...
COLORREF CHyperLink::g_crLinkColor		= RGB(0, 0, 128);
COLORREF CHyperLink::g_crActiveColor	        = RGB(128, 0, 0);
COLORREF CHyperLink::g_crVisitedColor	        = RGB(128, 0, 0);
COLORREF CHyperLink::g_crHoverColor		= RGB(0, 0, 255);
...
{		    
	...
	if (!BITSET(m_dwStyle,StyleNoHandCursor)) SetDefaultCursor();      // Try to load an "hand" cursor
...
void CHyperLink::SetColors(COLORREF crLinkColor, COLORREF crActiveColor, COLORREF crVisitedColor, COLORREF crHoverColor /* = -1 */) 
{
...
BOOL CHyperLink:: ModifyLinkStyle(DWORD dwRemove, DWORD dwAdd, BOOL bApply /* =TRUE */)
{
...
{	
	...
	if (!BITSET(m_dwStyle,StyleAutoSize)) return;
	...
	if (pParent) pParent->ScreenToClient(rcWnd);
	...
	if (BITSET(dwStyle, SS_CENTERIMAGE))	rcWnd.DeflateRect(0, (rcWnd.Height() - Extent.cy) / 2);
	else					rcWnd.bottom = rcWnd.top + Extent.cy;

	if (BITSET(dwStyle, SS_CENTER))		rcWnd.DeflateRect((rcWnd.Width() - Extent.cx) / 2, 0);
	else if (BITSET(dwStyle,SS_RIGHT))	rcWnd.left  = rcWnd.right - Extent.cx;
	else					rcWnd.right = rcWnd.left + Extent.cx;
...
void	CHyperLink::SetDefaultCursor()
{
	if (g_hLinkCursor == NULL)		// No cursor handle - load our own
	{
		...
		if (hModule)
		{
			...
			if (hHandCursor) g_hLinkCursor = CopyCursor(hHandCursor);
		}
		FreeLibrary(hModule);
	}
}
...
{	
	HINSTANCE	result = ShellExecute(NULL, _T("open"), m_strURL, NULL, NULL, SW_SHOW);
