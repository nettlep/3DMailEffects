...
void	Clipper::nClip(point4 &dst, const point4 &on, const point4 &off, point2 &tdst, const point2 &ton, const point2 &toff)
{
	float	delta = -on.z() / (off.z() - on.z());
	dst = on + (off - on) * delta;
...
void	Clipper::fClip(point4 &dst, const point4 &on, const point4 &off, point2 &tdst, const point2 &ton, const point2 &toff)
{
	float	delta = (on.w() - on.z()) / ((off.z() + on.w()) - (on.z() + off.w()));
	dst = on + (off - on) * delta;
	tdst = ton + (toff - ton) * delta;
}
...
void	Clipper::lClip(point4 &dst, const point4 &on, const point4 &off, point2 &tdst, const point2 &ton, const point2 &toff)
{
	float	delta = (-on.x() - on.w()) / (off.w() - on.w() + off.x() - on.x());
	dst = on + (off - on) * delta;
	tdst = ton + (toff - ton) * delta;
}
...
void	Clipper::rClip(point4 &dst, const point4 &on, const point4 &off, point2 &tdst, const point2 &ton, const point2 &toff)
{
	float	delta = (on.x() - on.w()) / (off.w() - on.w() - off.x() + on.x());
	dst = on + (off - on) * delta;
	tdst = ton + (toff - ton) * delta;
...
void	Clipper::tClip(point4 &dst, const point4 &on, const point4 &off, point2 &tdst, const point2 &ton, const point2 &toff)
{
	float	delta = (on.y() - on.w()) / (off.w() - on.w() - off.y() + on.y());
	dst = on + (off - on) * delta;
	tdst = ton + (toff - ton) * delta;
}
...
void	Clipper::bClip(point4 &dst, const point4 &on, const point4 &off, point2 &tdst, const point2 &ton, const point2 &toff)
{
	float	delta = (-on.y() - on.w()) / (off.w() - on.w() + off.y() - on.y());
	dst = on + (off - on) * delta;
	tdst = ton + (toff - ton) * delta;
...
unsigned int	Clipper::neClip(primitive<> &src)
{
	primitive<>	dst;
	vert<>		tmp;
...
unsigned int	Clipper::feClip(primitive<> &src)
{
	primitive<>	dst;
	vert<>		tmp;
...
unsigned int	Clipper::leClip(primitive<> &src)
{
	primitive<>	dst;
	vert<>		tmp;
...
unsigned int	Clipper::reClip(primitive<> &src)
{
	primitive<>	dst;
	vert<>		tmp;
...
unsigned int	Clipper::teClip(primitive<> &src)
{
	primitive<>	dst;
	vert<>		tmp;
...
unsigned int	Clipper::beClip(primitive<> &src)
{
	primitive<>	dst;
	vert<>		tmp;
...
bool	Clipper::clipPrimitive(primitive<> &p)
{
